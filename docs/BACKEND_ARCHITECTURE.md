...
параметры профиля репозитория (`EMBEDDING_MODEL`, лимиты чанка и retrieval)
имеют значения по умолчанию и появлялись вместе с кодом, который их читает.
Размерность векторов — не настройка, а константа `EMBEDDING_DIMENSION` в
домене: она задаёт тип столбца миграцией, и её смена — отдельная миграция
с перевложением чанков.
