...
**Чем рискуем** — во что обойдётся откат, если мердж окажется ошибкой. Дёшево: правку можно
вернуть реверт-коммитом. Дорого: миграция без рабочего `downgrade`, удаление колонки, смена
формата хранения — данные обратно не собрать. Пишется прямо, а не подразумевается.
